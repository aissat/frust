...
pub async fn handle_request(request_unique: RustRequestUnique) -> RustResponseUnique {
    ...
    let rust_response = match rust_resource {
        messages::counter_tuto::ID => sample_functions::handle_counter_number(rust_request).await,
        messages::increasing_number::ID => {
            sample_functions::handle_increasing_number(rust_request).await
        }
